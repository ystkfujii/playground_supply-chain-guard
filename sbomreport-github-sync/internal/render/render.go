...
func RenderSbomReports(reports []unstructured.Unstructured, opts Options) ([]File, error) {
	...
	for _, report := range reports {
		payload, err := payloadFor(report, opts.Content)
		if err != nil {
		...
		filePath := reportFilePath(prefix, report)
		files = append(files, File{Path: filePath, Content: content})
		index.Reports = append(index.Reports, buildIndexEntry(report, filePath))
	}
...
func payloadFor(report unstructured.Unstructured, content string) (any, error) {
	switch content {
	case "resource":
		return report.Object, nil
	case "report":
		m, found, err := unstructured.NestedMap(report.Object, "report")
		if err != nil {
			return nil, err
		}
		if !found {
	...
	case "cyclonedx":
		...
		return m, nil
	default:
		return nil, fmt.Errorf("unknown content mode %q", content)
	}
}

func buildIndexEntry(report unstructured.Unstructured, filePath string) indexEntry {
...
func reportFilePath(prefix string, report unstructured.Unstructured) string {
	ns := sanitize(report.GetNamespace())
	name := sanitize(report.GetName())
	if ns == "" {
		ns = "_cluster"
	}
	return path.Join(prefix, ns, name+".json")
}
